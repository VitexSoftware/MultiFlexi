...
class RunTemplateStatsCards extends \Ease\TWB4\Row
{
    ...
    public function __construct(\MultiFlexi\RunTemplate $runtemplate)
    {
        ...
        // Total Jobs Card
        $cardsRow->addColumn(3, $this->createCompactStatCard(
            '📊',
        ...
        if ($createdRaw || $updatedRaw) {
            if ($timingParts) { $metaWrap->addItem('<br>'); }
            if ($createdRaw) {
                $cd = new \DateTime($createdRaw);
                $metaWrap->addItem('📅 '._('Created').': '.$cd->format('Y-m-d H:i').' - '.$this->formatAge($cd));
            }
            if ($updatedRaw) {
                if ($createdRaw) { $metaWrap->addItem(' | '); }
                $ud = new \DateTime($updatedRaw);
                $metaWrap->addItem('💾 '._('Updated').': '.$ud->format('Y-m-d H:i').' - '.$this->formatAge($ud));
            }
        }
        ...
        if ($interval !== '') { $chips[] = '⏳ '._('Interval').': '.$this->formatInterval($interval); }
        
        $cron = (string) $this->runtemplate->getDataValue('cron');
        if ($cron !== '') { $chips[] = '🧭 Cron: '.$cron; }
        
        $delay = (int) ($this->runtemplate->getDataValue('delay') ?? 0);
        $chips[] = '⏱ '. _('Startup Delay').': '.$this->formatDuration($delay);
        
        $executor = (string) $this->runtemplate->getDataValue('executor');
        if ($executor !== '') { $chips[] = '⚙️ '._('Executor').': '.$executor; }
     ...
     */
    private function formatInterval(string $code): string
    {
        $map = [
            'n' => _('Manual'),
            'h' => _('Hourly'),
     ...
     */
    private function formatAge(\DateTime $date): string
    {
     ...
     */
    private function createCompactStatCard(string $icon, string $value, string $label, string $context): \Ease\TWB4\Card
    {
