    ...
    public function __construct(\MultiFlexi\Company $company, \MultiFlexi\Application $application, array $properties = [], array $linkProperties = [])
    {
        ...
        if ($runtemplatesRaw->count()) {
            foreach ($runtemplatesRaw as $runtemplateData) {
                ...
                $runtemplatesDiv->addItem(new \Ease\Html\SpanTag([new \Ease\Html\ATag('runtemplate.php?id='.$runtemplateData['id'], '⚗️#'.$runtemplateData['id'], ['class' => 'btn btn-inverse btn-sm']), $companyAppStatus], ['class' => 'btn-group', 'role' => 'group']));
            }
        } else {
