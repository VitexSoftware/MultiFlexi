...
class UserDataCorrectionForm extends Form
{
     ...
     * @param User|null $currentUser Current logged in user (null = same as user)
     */
    ...
    private function addPersonalDataField(string $fieldName, string $label, string $inputType, bool $requiresApproval): void
    {
        ...
        $canEditDirectly = $this->canEditDirectly && (!$requiresApproval || $this->isAdmin($this->currentUser));
        ...
        $labelContent = $label;
        if ($requiresApproval && !$this->isAdmin($this->currentUser)) {
        ...
        if ($canEditDirectly) {
            ...
            // Add validation
            $this->addValidationAttributes($input, $fieldName, $inputType);
            ...
            if ($requiresApproval) {
                ...
                $this->addValidationAttributes($newInput, $fieldName, $inputType);
     ...
     */
    private function addValidationAttributes(InputTag $input, string $fieldName, string $inputType): void
    {
        switch ($fieldName) {
            case 'email':
    ...
    private function showPendingRequests(): void
    {
        ...
        if (!empty($pendingRequests)) {
            ...
            foreach ($pendingRequests as $request) {
                ...
                if ($request['reviewer_notes'] && in_array($request['status'], ['approved', 'rejected'])) {
     ...
     */
    private function getStatusBadge(string $status): Badge
    {
        switch ($status) {
            case 'pending':
        ...
        switch ($fieldName) {
            ...
            case 'lastname':
                if (strlen($value) < 2 || strlen($value) > 50 || !preg_match('/^[A-Za-z\s\-\'\.]+$/', $value)) {
                    $this->addStatusMessage(
                        sprintf(_('Invalid %s: only letters, spaces, hyphens, and apostrophes allowed (2-50 characters)'), 
