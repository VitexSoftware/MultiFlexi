...
class DataExportSecurityManager extends \MultiFlexi\Engine
{
    ...
    public function canRequestExport(int $userId, string $ipAddress): array
    {
        ...
        if (!self::isUserAuthenticated($userId)) {
            return [
                'allowed' => false,
        ...
        // Check rate limiting
        if (!self::checkRateLimit($userId, $ipAddress)) {
            self::logSecurityEvent($userId, 'data_export_rate_limit_exceeded', $ipAddress);
        ...
        if (self::detectSuspiciousActivity($userId, $ipAddress)) {
            self::logSecurityEvent($userId, 'data_export_suspicious_activity', $ipAddress);
    ...
    public function createSecureToken(int $userId, string $format, string $ipAddress, string $userAgent): array
    {
        ...
        if ($tokenEngine->insertToSQL($tokenRecord)) {
            self::logGDPRAudit($userId, 'data_export_token_created', 'Article 15', [
                'format' => $format,
    ...
    public function verifyDownloadToken(string $token, int $userId, string $ipAddress)
    {
        ...
        if (!$tokenRecord) {
            self::logSecurityEvent($userId, 'data_export_invalid_token', $ipAddress);
        ...
        if (strtotime($tokenRecord['expires_at']) < time()) {
            self::logSecurityEvent($userId, 'data_export_expired_token', $ipAddress);
        ...
        if ($tokenRecord['used_at']) {
            self::logSecurityEvent($userId, 'data_export_reused_token', $ipAddress);
        ...
        self::logGDPRAudit($userId, 'data_export_token_used', 'Article 15', [
     ...
     * Check rate limiting.
     */
    private static function checkRateLimit(int $userId, string $ipAddress): bool
    {
        $rateLimitEngine = new \Ease\SQL\Engine();
        $rateLimitEngine->myTable = 'data_export_rate_limits';
     ...
     */
    private static function isUserAuthenticated(int $userId): bool
    {
     ...
     */
    private static function detectSuspiciousActivity(int $userId, string $ipAddress): bool
    {
     ...
     * Log security event.
     */
    private static function logSecurityEvent(int $userId, string $event, string $ipAddress, array $details = []): void
    {
     ...
     */
    private static function logGDPRAudit(int $userId, string $action, string $article, array $details = []): void
    {
