...
RUN apt-get update && apt-get install -y locales php7.3-sqlite  cron locales-all && rm -rf /var/lib/apt/lists/* \
...
RUN apt install multi-flexibee-setup
RUN apt install flexibee-matcher 
RUN apt install flexibee-reminder 
RUN apt install flexibee-contract-invoices 
RUN apt install flexibee-digest
